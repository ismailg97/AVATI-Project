...
namespace TestProject
{
    ...
    class UiTestUpdateSkill<T> where T : IWebDriver, new()
    {
        ...
        public void UpdateSkillTest() {
            ...
            var updatebtn = _driver.FindElement(By.Id("Update-C#"));
            updatebtn.Click();
            Thread.Sleep(2000);

            var skillname = _driver.FindElement(By.Id("skillname"));
            skillname.SendKeys("TestUpdate");
            ...
            foreach (IWebElement row in tableRow)
            {
                ...
                if (rowTD.Count() >= 2)
                {
                    if (rowTD[0].Text.Equals("TestUpdate") && rowTD[1].Text.Equals(Skilltype.Softskill.ToString()))
                        success = true;
                }
            }
        ...
        {
            var updatebtn = _driver.FindElement(By.Id("Update-TestUpdate"));
            updatebtn.Click();
            Thread.Sleep(2000);

            var skillname = _driver.FindElement(By.Id("skillname"));
            skillname.SendKeys("C#");
            
            IWebElement skillkat = _driver.FindElement(By.Id("skillkategorie"));
            SelectElement selectkat = new SelectElement(skillkat);
            selectkat.SelectByText("Hardskill");
